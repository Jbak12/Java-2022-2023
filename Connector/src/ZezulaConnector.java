...
class ZezulaConnector implements NetConnection{
    ...
    public void connect(String host, int port){
        try(Socket socket = new Socket(host, port)){
            PrintWriter DataPrintWriter = new PrintWriter(socket.getOutputStream(), true);
            BufferedReader DataBuffReader = new BufferedReader(new InputStreamReader(socket.getInputStream()));
    ...
    public static void main (String[] args){
        ZezulaConnector c = new ZezulaConnector();
        c.connect("172.30.24.101", 8080);
    }
}